docker:
  containers:
    builder-openwrt-v812b02b-bb-ar71xx:
      image: wlanslovenija/openwrt-builder
      tag: v812b02b_bb_ar71xx
      environment:
        BUILDER_PUBLIC_KEY: ssh-rsa  builder@wlan-si.net
        VIRTUAL_HOST: packages.wlan-si.net
        VIRTUAL_URL: /firmware/git.812b02b/openwrt/barrier_breaker/ar71xx/
    builder-openwrt-v812b02b-cc-ar71xx:
      image: wlanslovenija/openwrt-builder
      tag: v812b02b_cc_ar71xx
      environment:
        BUILDER_PUBLIC_KEY: ssh-rsa  builder@wlan-si.net
        VIRTUAL_HOST: packages.wlan-si.net
        VIRTUAL_URL: /firmware/git.812b02b/openwrt/chaos_calmer/ar71xx/
