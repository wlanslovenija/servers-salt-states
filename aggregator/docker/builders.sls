docker:
  containers:
    # Barrier Breaker
    builder-openwrt-v6a58c59-bb-ar71xx:
      image: wlanslovenija/openwrt-builder
      tag: v6a58c59_bb_ar71xx
      environment:
        BUILDER_PUBLIC_KEY: ssh-rsa  builder@wlan-si.net
        VIRTUAL_HOST: packages.wlan-si.net
        VIRTUAL_URL: /firmware/git.6a58c59/openwrt/barrier_breaker/ar71xx/
    builder-openwrt-v6a58c59-bb-lantiq:
      image: wlanslovenija/openwrt-builder
      tag: v6a58c59_bb_lantiq
      environment:
        BUILDER_PUBLIC_KEY: ssh-rsa  builder@wlan-si.net
        VIRTUAL_HOST: packages.wlan-si.net
        VIRTUAL_URL: /firmware/git.6a58c59/openwrt/barrier_breaker/lantiq/
    builder-openwrt-v6a58c59-bb-x86:
      image: wlanslovenija/openwrt-builder
      tag: v6a58c59_bb_x86
      environment:
        BUILDER_PUBLIC_KEY: ssh-rsa  builder@wlan-si.net
        VIRTUAL_HOST: packages.wlan-si.net
        VIRTUAL_URL: /firmware/git.6a58c59/openwrt/barrier_breaker/x86/
    builder-openwrt-v6a58c59-bb-x86_hyperv:
      image: wlanslovenija/openwrt-builder
      tag: v6a58c59_bb_x86_hyperv
      environment:
        BUILDER_PUBLIC_KEY: ssh-rsa  builder@wlan-si.net
        VIRTUAL_HOST: packages.wlan-si.net
        VIRTUAL_URL: /firmware/git.6a58c59/openwrt/barrier_breaker/x86_hyperv/
    # Chaos Calmer
    builder-openwrt-v3db9e4a-cc-ar71xx:
      image: wlanslovenija/openwrt-builder
      tag: v3db9e4a_cc_ar71xx
      environment:
        BUILDER_PUBLIC_KEY: ssh-rsa  builder@wlan-si.net
        VIRTUAL_HOST: packages.wlan-si.net
        VIRTUAL_URL: /firmware/git.3db9e4a/openwrt/chaos_calmer/ar71xx/
    builder-openwrt-v3db9e4a-cc-lantiq:
      image: wlanslovenija/openwrt-builder
      tag: v3db9e4a_cc_lantiq
      environment:
        BUILDER_PUBLIC_KEY: ssh-rsa  builder@wlan-si.net
        VIRTUAL_HOST: packages.wlan-si.net
        VIRTUAL_URL: /firmware/git.3db9e4a/openwrt/chaos_calmer/lantiq/
